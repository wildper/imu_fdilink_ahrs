...
ahrsBringup::processLoop() {
  ...
  while (rclcpp::ok()) {
    ...
    // 读取INSGPS数据进行解析，并发布相关话题
      ...
      odom_msg.twist.twist.linear.x = insgps_frame_.frame.data.data_pack.Velocity_North;  // Speed in the X direction
      odom_msg.twist.twist.linear.y = insgps_frame_.frame.data.data_pack.Velocity_East;   // Speed in the Y direction
