...
ahrsBringup::ahrsBringup() : rclcpp::Node("ahrs_bringup")
{
    ...
    this->declare_parameter<std::int64_t>("serial_baud_", 115200);
    ...
    Euler_angles_pub_ =
        create_publisher<geometry_msgs::msg::Vector3>(Euler_angles_topic.c_str(), 10);
...
void ahrsBringup::processLoop()
{
    ...
    while (rclcpp::ok())
    {
        ...
        if (if_debug_)
        {
            if (head_s != 1)
            {
                RCLCPP_ERROR(this->get_logger(),
                             "Read serial port time out! can't read pack head.");
            }
            std::cout << std::endl;
        ...
        if (head_type[0] != TYPE_IMU             //
            && head_type[0] != TYPE_AHRS         //
            && head_type[0] != TYPE_INSGPS       //
            && head_type[0] != TYPE_GEODETIC_POS //
            && head_type[0] != 0x50              //
            && head_type[0] != TYPE_GROUND && head_type[0] != 0xff)
        {
        ...
        if (head_type[0] == TYPE_IMU)
        {
            ...
            if (if_debug_)
            {
                ...
                std::cout << "head_crc16_h: " << std::hex << (int)head_crc16_h << std::dec
                          << std::endl;
                std::cout << "head_crc16_l: " << std::hex << (int)head_crc16_l << std::dec
                          << std::endl;
        ...
        else if (head_type[0] == TYPE_AHRS)
        {
            ...
            {
                std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec << std::endl;
                std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec
                          << std::endl;
                std::cout << "head_crc16_h: " << std::hex << (int)head_crc16_h << std::dec
                          << std::endl;
                std::cout << "head_crc16_l: " << std::hex << (int)head_crc16_l << std::dec
        ...
        else if (head_type[0] == TYPE_GEODETIC_POS)
        {
            ...
            uint16_t head_crc16 = head_crc16_l + (head_crc16_h << 8);
            size_t data_s = serial_.read(Geodetic_Position_frame_.read_buf.read_msg,
                                         (GEODETIC_POS_LEN + 1)); // 24+1
            uint16_t CRC16 =
                CRC16_Table(Geodetic_Position_frame_.frame.data.data_buff, GEODETIC_POS_LEN);
            if (if_debug_)
            {
                std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec << std::endl;
                std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec
        ...
        else if (head_type[0] == TYPE_INSGPS)
        {
            ...
            // odom_msg.child_frame_id = robot_frame_id; // Odometer TF subcoordinates
            odom_msg.twist.twist.linear.x =
                insgps_frame_.frame.data.data_pack.Velocity_North; // Speed in the X direction
            odom_msg.twist.twist.linear.y =
                insgps_frame_.frame.data.data_pack.Velocity_East; // Speed in the Y direction
...
void ahrsBringup::magCalculateYaw( //
    double roll, double pitch, double &magyaw, double magx, double magy, double magz)
{
    double temp1 = magy * cos(roll) + magz * sin(roll);
    double temp2 =
        magx * cos(pitch) + magy * sin(pitch) * sin(roll) - magz * sin(pitch) * cos(roll);
...
void ahrsBringup::checkSN(int type)
{
    ...
    case TYPE_GEODETIC_POS:
        if (++read_sn_ != Geodetic_Position_frame_.frame.header.serial_num)
        {
            ...
            {
                sn_lost_ +=
                    256 - (int)(read_sn_ - Geodetic_Position_frame_.frame.header.serial_num);
                if (if_debug_)
                {
                    RCLCPP_WARN(this->get_logger(), "detected sn lost.");
                }
